...
//     progress.attempts += 1;
//     progress.status = allTestsPassed ? 'completed' : 'in_progress';
...
// server/controllers/challenge.controller.js
const Challenge = require('../models/challenge.model');
const Progress = require('../models/progress.model');
const AppError = require('../utils/appError');
const catchAsync = require('../utils/catchAsync');
...
exports.deleteChallenge = catchAsync(async (req, res, next) => {
  ...
  await Progress.deleteMany({ challenge: req.params.id });
...
exports.submitSolution = catchAsync(async (req, res, next) => {
  ...
  for (const testCase of challenge.testCases) {
    try {
      ...
      const wrappedCode = `
        ...
        const solution = memoize;
        
        // Execute function with the test input
        const fn = ${testCase.input[0].toString()};
        const args = ${JSON.stringify(testCase.input[1])};
        const result = solution(fn)(...args);
        result;
      `;
  ...
  if (!progress) {
    progress = new Progress({
      ...
      status: allTestsPassed ? 'completed' : 'in_progress',
      completedAt: allTestsPassed ? Date.now() : undefined,
      timeSpent: 0, // Will be updated by the frontend
    });
  } else {
    progress.submittedCode = code;
    progress.attempts += 1;
